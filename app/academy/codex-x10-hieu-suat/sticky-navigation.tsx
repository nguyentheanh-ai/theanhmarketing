...
export function StickyNavigation({ price = CODEX_AGENT_KIT_OFFER.price }: { price?: number }) {
  ...
  return <>
    <aside ref={bar} className="cx-sticky" hidden={formVisible} aria-label="Đăng ký khóa Codex">
      <div className="cx-sticky-inner">
        ...
        <div className="cx-sticky-price"><span>Giá chính thức</span><strong>{money(price)}</strong><s aria-label="Giá gốc">{money(CODEX_AGENT_KIT_OFFER.originalPrice)}</s></div>
        <div className="cx-sticky-actions">
          ...
          <a className="cx-btn" href="#dang-ky" onClick={e => go(e, "dang-ky")}>Đăng ký ngay · {money(price)} <span aria-hidden>↗</span></a>
        </div>
      </div>
    </aside>
