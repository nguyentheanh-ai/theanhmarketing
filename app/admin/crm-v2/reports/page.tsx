...
export default async function CrmV2ReportsPage({ searchParams }: PageProps) {
  await requireAdminAuth("/admin/crm-v2/reports", ["owner"]);
  ...
  const metrics: KpiMetric[] = [
    ...
    { label: "Đơn thanh toán / lead trong kỳ", value: ratio(paid, leads), tone: "blue", series: [] },
    ...
    { label: "Chi phí / khách mua trong kỳ", value: ads.available && paidCustomers ? compactMoney(spend / paidCustomers) : "Chưa đủ dữ liệu", tone: "orange", series: [] },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-sm">
        <StatusBadge tone={ads.available ? (ads.quality.status === "partial" ? "orange" : "green") : "red"}>{ads.available ? (ads.quality.status === "partial" ? "Ads · số liệu tạm tính" : "Ads đã chốt") : "Ads chưa khả dụng"}</StatusBadge>
        <span>Khoảng báo cáo: {query.range}. Doanh thu theo thời điểm thanh toán; lead theo thời điểm tạo trong CRM. Đơn/lead chưa phải tỷ lệ chuyển đổi cùng nhóm khách; “Doanh thu sau Ads” chưa phải lợi nhuận kế toán.</span>
      </div>
      ...
      <section className="space-y-3">
        ...
        <CrmDataTable rows={snapshot.attributionRows.rows} columns={[
          ...
          { key: "cr", label: "Đơn / lead trong kỳ" }, { key: "revenue", label: "Doanh thu" }, { key: "note", label: "Ghi chú" },
        ]} />
      </section>
    </div>
  );
