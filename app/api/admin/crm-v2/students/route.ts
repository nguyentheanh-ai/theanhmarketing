...
export async function GET(request: Request) {
  const rateLimit = checkRateLimit({
    key: rateLimitKey(request, "admin:crm-v2:students"),
    limit: 120,
    windowMs: 10 * 60 * 1000,
  });
  ...
  if (!(await requireOwner())) {
    return NextResponse.json({ ok: false, message: "Ban khong co quyen xem CRM v2." }, { status: 403 });
  }
  ...
  const result = await listCrmV2Students(query);
