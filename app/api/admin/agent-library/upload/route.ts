...
export async function POST(request: Request) {
  ...
  const entry = catalog.agents.find(agent=>agent.slug===body?.slug);
  ...
  try {
    ...
    if (bucket.error && String((bucket.error as {status?:number;statusCode?:string}).statusCode ?? (bucket.error as {status?:number}).status) === "404") {
      const created = await client.storage.createBucket(bucketName,{public:false,fileSizeLimit:52428800,allowedMimeTypes:["application/zip","application/x-zip-compressed"]});
