...
export function HomeCourseHighlights({ courses }: { courses: Course[] }) {
  ...
  return <>
    {facebook?.landingPageUrl ? <section className={styles.feature} aria-labelledby="facebook-feature-title" data-reveal>
      <div className={styles.featureCopy}>
        <p className={styles.eyebrow}>02 / FACEBOOK ADS THỰC HÀNH</p>
      ...
      <div className={styles.featureVisual}>
        <Image src="/doi-ngu-nhan-su-ai/images/generated/role-ads.webp" alt="Thế Anh đối chiếu kế hoạch và nội dung quảng cáo" width={1400} height={1050} sizes="(min-width: 1024px) 45vw, 100vw" className={styles.featureImage} />
    ...
    <section className={styles.highlights} aria-labelledby="highlights-title">
      <div className={styles.heading} data-reveal><div><p className={styles.eyebrow}>THÊM CÔNG CỤ CHO CÔNG VIỆC</p><h2 id="highlights-title">Học kiến thức.<br />Có công cụ để làm tiếp.</h2></div><p className={styles.sectionIntro}>Chọn chương trình phù hợp với việc anh/chị đang cần giải quyết.</p></div>
      <div className={styles.highlightGrid}>{highlights.map(item => {
        ...
        return <article key={item.slug} className={styles.highlightCard} data-reveal>
          <div className={styles.highlightPhoto}><Image src={`/doi-ngu-nhan-su-ai/images/generated/${item.image}`} alt={item.label} fill sizes="(min-width: 768px) 220px, 100vw" /></div><div className={styles.highlightContent}>
          ...
          <Link href={course.landingPageUrl} className={styles.textLink}>Khám phá chương trình <ArrowRight size={17} /></Link>
        </div></article>;
      })}</div>
    </section>
  </>;
