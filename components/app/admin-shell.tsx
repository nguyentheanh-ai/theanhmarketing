        ...
        <nav className="mt-10 grid gap-2">
          {adminNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-4 py-3 text-sm font-semibold text-white/60 transition hover:bg-white hover:text-black"
            >
              {item.label}
            </Link>
          ))}
        ...
        <nav className="mt-4 flex gap-2 overflow-x-auto">
          {adminNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="shrink-0 rounded-full bg-white px-4 py-2 text-sm font-semibold text-black/60"
            >
              {item.label}
            </Link>
          ))}
