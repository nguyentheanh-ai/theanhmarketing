...
test('LMS summary preserves active identity, expiry and progress without reading lesson content/resources', async()=>{
 ...
 const client={from(table){calls.push(table);assert.equal(table,'courses');const q={select(s){assert.ok(!s.includes('*'));assert.ok(!s.includes('lesson_resources'));return q;},order(){return q;},then(resolve){return Promise.resolve({data:rows,error:null}).then(resolve);}};return q;},rpc:async(name)=>{assert.equal(name,'crm_v2_lms_enrollments_raw');return {data:{enrollments,progress:[{enrollment_id:'e1',lesson_id:'l1',status:'completed'}]},error:null};}};
